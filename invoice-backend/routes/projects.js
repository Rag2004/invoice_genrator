...
// ============================================================================
// routes/projects.js - UPDATED WITH CLIENT EMAIL SUPPORT
// ============================================================================
...
/**
 * GET /api/projects/:code/setup
 * ✅ UPDATED: Returns project + client (with email) + consultant
 * Uses Apps Script getInvoiceSetup which now includes client email
 * Returns: { ok, project, client, consultant }
 */
router.get('/:code/setup', async (req, res) => {
  const rawCode = req.params.code || '';
  ...
  try {
    ...
    // ----- REMOTE MODE (Apps Script) -----
    // Call Apps Script action getInvoiceSetup
    const result = await apps.getInvoiceSetup(code);
    ...
    logger.info(
      {
        ...
        resultKeys: result ? Object.keys(result) : null,
        ok: result && result.ok,
        hasClientEmail: !!result?.client?.email
      },
      'Apps Script getInvoiceSetup response'
    );
    ...
    // ✅ WARN if client email is missing
    if (client && !client.Client_email && !client.email) {
      logger.warn(
        { 
          projectCode: code, 
          clientCode: client.Client_Code || client.code 
        }, 
        '⚠️ Client email missing in project setup'
      );
    }
