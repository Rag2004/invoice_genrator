...
// ============================================================================
// PDF DOWNLOAD - Server-side Puppeteer rendering
// ============================================================================

/**
 * Download invoice PDF generated by the server using Puppeteer.
 * Calls GET /api/invoices/:invoiceId/pdf and triggers browser download.
 *
 * @param {string} invoiceId - ID of the finalized invoice
 * @param {string} [filename] - desired filename for the download
 * @returns {Promise<void>}
 */
export async function downloadInvoicePDFFromServer(invoiceId, filename) {
  if (!invoiceId) throw new Error('Invoice ID is required');
  ...
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 60000); // 60s timeout for PDF gen
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      let errMsg = `PDF download failed (${resp.status})`;
      try {
    ...
    clearTimeout(timeoutId);
    if (err.name === 'AbortError') {
      throw new Error('PDF generation timed out (60s). Please try again.');
    }
    throw err;
  }
}