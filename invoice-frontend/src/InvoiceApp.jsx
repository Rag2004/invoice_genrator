...
export default function InvoiceApp() {
    ...
    getTeam()
      .then((t) => {
        ...
        const normalized = items.map((x) => ({
          ...
          defaultMode: x.defaultMode || x.mode || 'Online',
          defaultRate: Number(
            x.Hourly_rate || x.hourlyRate || x.defaultRate || 0
          ),
          baseFactor: Number(
            x.baseFactor ?? x.BaseFactor ?? x.Factor ?? x.factor ?? 1
          ),
        }));
        setTeamOptions(normalized);
      })
  ...
  useEffect(() => {
    ...
    getProject(code)
      .then((res) => {
        ...
        const clientCodeFromProject =
          p.clientCode ?? p.Client_Code ?? p.client_code ?? p.client;
        ...
        const hourlyRateFromProject =
          p.hourlyRate ?? p.Hourly_rate ?? p.Hourly_Rate ?? p.hourly_rate;

        const serviceFeePctRaw =
          p.serviceFeePct ?? p.service_fee_pct ?? p.ServiceFeePct;
        ...
        setInvoice((prev) => ({
          ...
          consultantId: consultantIdFromProject || prev.consultantId,
          serviceFeePct:
            typeof svcPct === 'number' ? svcPct : prev.serviceFeePct,
          baseHourlyRate:
            hourlyRateFromProject !== undefined &&
            hourlyRateFromProject !== null
              ? Number(hourlyRateFromProject || 0)
              : prev.baseHourlyRate,
          billingAddress: prev.billingAddress || defaultBillingAddress || '',
        }));
  ...
  useEffect(() => {
    ...
    getClient(code)
      .then((c) => {
        ...
        // Only use client billing address if we *don't* already have one
        if (c?.billingAddress) {
          setInvoice((prev) => ({
            ...prev,
            billingAddress: prev.billingAddress || c.billingAddress,
  ...
  const handleValidateInvoice = () => {
    ...
    const hasHours = (invoice.items || []).some(
      (it) => Number(it.hours || 0) > 0
    );
    if (!hasHours) {
      return { ok: false, err: 'Add at least one team member with hours.' };
    }
  ...
  const loadDraftById = async (invoiceId) => {
    ...
    try {
      ...
      const serviceFeePctPercent = incomingServiceFeeToPercent(
        data.serviceFeePct
      );
